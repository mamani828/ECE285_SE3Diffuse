...
def parse_args() -> TrainConfig:
    ...
    for field in fields(TrainConfig):
        ...
        if isinstance(default_value, bool):
            ...
            parser.add_argument(arg_name, type=float, default=default_value)
...
def sample_ar1_noise(batch_size: int,horizon: int,dim: int,rho: float,device: torch.device,dtype: torch.dtype,) -> torch.Tensor:
    ...
    sigma = math.sqrt(max(1.0 - rho * rho, 1e-12))
    for t in range(1, horizon):
        eta_t = torch.randn(batch_size, dim, device=device, dtype=dtype)
        eps[:, t] = rho * eps[:, t - 1] + sigma * eta_t
...
def run_epoch(
    ...
    for batch in loader:
        ...
        noise = sample_diffusion_noise_like(
            u0,
            use_correlated_noise=cfg.use_correlated_noise,
            rho=cfg.noise_rho_train,
            corr_type=cfg.noise_corr_type,
        )
...
def main() -> None:
    ...
    print(f"saving outputs to: {os.path.abspath(cfg.save_dir)}")
    print(f"dataset horizon={horizon} dt={dt} v_max={v_max} w_max={w_max} map_mode={cfg.map_mode}")
    print(
        f"correlated_noise={cfg.use_correlated_noise} "
        f"type={cfg.noise_corr_type} "
        f"rho_train={cfg.noise_rho_train} "
        f"rho_sample={cfg.noise_rho_sample}"
    )
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(
            f"epoch {epoch:03d} | "
            f"train loss {train_metrics['loss']:.5f} noise {train_metrics['noise']:.5f} "
            f"state {train_metrics['state']:.5f} term {train_metrics['terminal']:.5f} | "
            f"val loss {val_metrics['loss']:.5f} noise {val_metrics['noise']:.5f} "
            f"state {val_metrics['state']:.5f} term {val_metrics['terminal']:.5f}"
        )
...
if __name__ == "__main__":
    print("Starting training with correlated diffusion noise...")
    main()